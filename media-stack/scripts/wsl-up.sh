...
PRISMARR_PORT="${PRISMARR_HTTP_PORT:-7070}"
grep -q '^PRISMARR_HTTP_PORT=' .env 2>/dev/null && PRISMARR_PORT="$(grep -E '^PRISMARR_HTTP_PORT=' .env | cut -d= -f2- | tr -d '\r')"
...
echo "  http://${HOST}:${PRISMARR_PORT}/          — Prismarr"
